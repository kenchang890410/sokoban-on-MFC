...
BEGIN_MESSAGE_MAP(CSokobanApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CSokobanApp::OnAppAbout)
	// 依據文件命令的標準檔案
...
CSokobanApp::CSokobanApp() noexcept
{
	...
	SetAppID(_T("Sokoban.AppID.NoVersion"));
...
BOOL CSokobanApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CSokobanDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架視窗
		RUNTIME_CLASS(CSokobanView));
	if (!pDocTemplate)
		return FALSE;
